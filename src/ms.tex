...
by a factor of five to ten in sensitivity and it will be able to observe the whole sky from a combination of two sites:
a northern site in La Palma, Spain, and a southern one in Paranal, Chile.
CTA will be able to observe gamma rays in the energy range between $20\,\mathrm{GeV}$ and $300\,\mathrm{GeV}$
...
\begin{enumerate}
  \item In the low-level analysis, the properties of the recorded air shower events
    ...
    The raw data consists of very short (\textasciitilde $40\,\mathrm{ns}$) videos recorded with the fast and
  ...
  \item In the higher-level analysis, these reconstructed event lists are used together with the
    characterization of the measurement process and the low-level analysis to perform the actual scientific analysis.
    This includes energy spectra, light curves, sky maps and many more.
\end{enumerate}

A prototype for the low-level analysis is ctapipe~\cite{ctapipe},
...
The high-level analysis will be based on the Gammapy~\cite{gammapy} library, which
will be the core of the CTA science tools.

Both Gammapy and \texttt{ctapipe} are using astropy heavily, mainly for units, coordinate transformations,
tables and FITS IO.
...
a total of 107 merged pull requests were contributed to ctapipe.
