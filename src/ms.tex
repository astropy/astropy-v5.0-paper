...
The Astropy-ii paper discussed the {\it Learn Astropy} effort only in the
context of online tutorials and guides. This vision has been realized as
{\it Learn Astropy} has released a new version of this website, and has
...
\item {\bf User forums:} In addition to the pre-existing Astropy mailing list,
and the Python in Astronomy Facebook group, users have been invited to the
Astropy Slack workspace. The Moore Foundation grant allows this Slack space to
be on a paid plan; additionally NumFOCUS has negotiated a special rate for
open-source projets. There is some internal debate amongst Astropy developers
on whether Astropy Slack should be reserved for developer discussions, with
users redirected to another site. Accordingly, the Astropy Project has commissioned
