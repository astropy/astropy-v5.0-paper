...
projects via the \texttt{sphinx-astropy} meta package. This tool provides 
a pre-configured Sphinx setup compatible with Astropy projects, which 
includes several extensions useful for generating API documentation 
(\texttt{sphinx-automodapi}), allowing for Numpy docstring parsing 
(\texttt{numpydoc}), embedded image handling (\texttt{sphinx-gallery}; 
\texttt{pillow}), advanced documentation testing support 
(\texttt{pytest-doctestplus}), and providing a custom documentation theme 
ideal for analysis packages (\texttt{astropy-sphinx-theme}).

Community package testing infrastructure is supported through the 
\texttt{pytest-astropy} meta-package, providing a unified testing framework 
with useful extensions compatible with both Astropy- and 
non-Astropy-affiliated community packages. This meta-package pulls in several 
\texttt{pytest} plugins to help with custom test headers 
(\texttt{pytest-astropy-header}), accessing remotely-hosted data files in 
tests (\texttt{pytest-remotedata}), interoperability with documentation 
(\texttt{pytest-doctestplus}), dangling file handle checking 
(\texttt{pytest-openfiles}), data array comparison support in tests 
(\texttt{pytest-arraydiff}), sub-package command-line testing support 
(\texttt{pytest-filter-subpackage}), improved mock object testing 
(\texttt{pytest-mock}), test coverage reports and measurements 
(\texttt{pytest-cov}), and configuring package for property-based 
testing (\texttt{hypothesis}).
