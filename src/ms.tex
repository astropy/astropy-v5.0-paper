...
\subsubsection*{Interoperability} \label{sec:core-features-interoperability}
  ...
  \paragraph{Astropy Fits in Time}    % IN PROGRESS (ADAPTING FROM THE WHATSNEW).

    As anyone who has worked with FITS files knows, the FITS standard is both
    detailed and very flexible. While this means FITS is useful across many
    domains, working with the files can be involved. Expanding on the WCS and
    related work, the \astropyfits package now provides support for reading and
    writing native \astropyTime table columns using the FITS Time standard
    \citep{FITS-Time:2015}. For backward compatibility, this feature may be
    turned off. Implementation of this feature was made possible through the
    support of the Google Summer of Code 2017 program.
    % TODO: is there a way to cite this
  ...
  \paragraph{Table Mixin Columns}
    ...
    \begin{itemize}
      ...
      \item Added support for \package{dask} arrays in tables
  ...
  % partially driven by this?
    The improvements to \astropy's I/O architecture are mirrored in the increased support for I/O options.
