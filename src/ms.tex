...
\subsubsection*{Interoperability} \label{sec:core-features-interoperability}
  ...
  % New features capture a lot of attention. While exciting, the arguably more
  % significant improvements are those of interoperability -- ensuring existing
  % features connect smoothly together.

  % MHvK: this repeats the same thing 3 times; try to use just one sentence!
  ...
  % Better list? Also better transition
  If done right, all a user notices is that code using many different python
  features and libraries ``just works''.  Here, we describe a few
  particular efforts towards this goal.
  ...
  \paragraph{\package{numpy} on Units}
    ...
    functions.
    % MHvK: I don't think the following is true, so commented it out.
    % These advances have similarly benefitted many
    % \package{numpy}-dependent packages, like \package{scipy} and their sparse
    % matrices. And again \astropypkg benefits since more \package{scipy}
    ...
    Request) would be sufficient to see this realized. For a few remaining functions,
  ...
  \paragraph{Astropy in Time and on Units}
    ...
    % Not only did \astropyunits gain powerful integrations with \package{numpy},
    % there were many intra-package improvements.
    % MHvK: suggested short replacement.
    Units were also integrated further within \astropypkg.
    ...
    % As this is a pseudo-unit an equivalency
    % \astropyapidoc{cosmology.units.dimensionless_redshift}{dimensionless\_redshift}
    % is added (and enabled by default) to allow for redshift - dimensionless
    % conversions.
    ...
    equivalencies,
    % : \astropyapidoc{cosmology.units.redshift_distance}{redshift\_distance},
    % \astropyapidoc{cosmology.units.redshift_hubble}{redshift\_hubble}, and
    % \astropyapidoc{cosmology.units.redshift_temperature}{redshift\_temperature},
    which may be used separately.

    % IN PROGRESS (ADAPTING FROM THE WHATSNEW).
    % MHvK: maybe separate \paragraph?
  ...
  % MHvK: maybe do this before the structure? Change in structure
  % partially driven by this?
...
\subsection*{Precision, Accuracy, \& Reproducibility} \label{sec:core-features-precision_accuracy_repreducibility}
  ...
  \paragraph{Time}
  % MHvK: pulsar timing cares about ns, as does VLBI!
...
% MHvK: Merged with the new feature section
